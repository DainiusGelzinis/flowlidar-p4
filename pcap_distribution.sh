#!/usr/bin/env bash
# pcap_distribution.sh — True flow-size distribution from a pcap. Ground truth
# for FlowLiDAR comparisons.
#
# First arg: numeric → limit to first N packets; non-numeric → use as pcap path.
# Second arg (optional): pcap path when first arg is a packet count.
#
# Usage:
#   ./pcap_distribution.sh                       # whole default pcap
#   ./pcap_distribution.sh 2300000               # first 2.3M packets, default pcap
#   ./pcap_distribution.sh 2300000 /path.pcap    # first N of custom pcap
#   ./pcap_distribution.sh /path.pcap            # whole custom pcap
...
TSHARK_ARGS=( -r "$PCAP" -T fields
              -e ip.src -e ip.dst -e ip.proto
              -e tcp.srcport -e tcp.dstport
              -e udp.srcport -e udp.dstport )
[[ -n "$LIMIT" ]] && TSHARK_ARGS+=( -c "$LIMIT" )
