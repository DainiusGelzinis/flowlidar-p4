#!/usr/bin/env python3
"""
FlowLiDAR Prototype 9 — Control Plane (Traditional BF + sub-sketch CMS)
...
# ---------------------------------------------------------------------------
# Hybrid fast I/O — delegate the slow bulk read+clear to the C++ helper.
# Returns dicts in the same shape as read_bf_snapshot / read_cms_snapshot so
# the rest of the postprocessing pipeline doesn't need to change.
# ---------------------------------------------------------------------------

def fastio_snapshot_and_clear(fastio_bin, snapshot_dir, pipe_id,
                              bf_ids, cms_ids):
    os.makedirs(snapshot_dir, exist_ok=True)
    cmd = [fastio_bin, 'snapshot_and_clear', snapshot_dir,
           '--bf',  ','.join(str(i) for i in bf_ids),
           '--cms', ','.join(str(i) for i in cms_ids)]
    if pipe_id is not None:
        cmd += ['--pipe', str(pipe_id)]
    t0 = time.time()
    subprocess.run(cmd, check=True)
    print(f'  fast_io snapshot+clear  : {time.time()-t0:.2f}s')

    # When called for the empty-flow-table case the function isn't asked to
    # build snapshots; the caller checks before calling. Still return dicts.
    bf_snapshot = {}
    for row in BF_ROWS:
        with open(os.path.join(snapshot_dir, f'{row}.bin'), 'rb') as f:
            bf_snapshot[row] = list(f.read())   # 1 byte per cell, 0/1
    ...
    for row in CMS_ROWS:
        with open(os.path.join(snapshot_dir, f'{row}.bin'), 'rb') as f:
            data = f.read()
        cms_snapshot[row]    = list(struct.unpack(f'<{len(data) // 2}H', data))
        cms_field_names[row] = f'SwitchIngress.{row}.f1'
    return bf_snapshot, cms_snapshot, cms_field_names
...
def process_epoch(epoch_num, flow_table, bfrt_info, target, total=0,
                  fastio_bin=None, snapshot_dir=None, pipe_id=None,
                  bf_table_ids=None, cms_table_ids=None):
    sep = '=' * 72
    print(f'\n{sep}')
    ...
    if not flow_table:
        ...
        if fastio_bin:
            fastio_snapshot_and_clear(fastio_bin, snapshot_dir, pipe_id,
                                       bf_table_ids, cms_table_ids)
        else:
    ...
    if fastio_bin:
        print('  Reading + clearing BF + CMS via fast_io (C++)...')
        bf_snapshot, cms_snapshot, cms_field_names = \
            fastio_snapshot_and_clear(fastio_bin, snapshot_dir, pipe_id,
                                       bf_table_ids, cms_table_ids)
        already_cleared = True
    else:
        print('  Reading BF + CMS registers (Python bfrt_grpc)...')
        t0 = time.time()
        bf_snapshot                   = read_bf_snapshot(bfrt_info, target)
        cms_snapshot, cms_field_names = read_cms_snapshot(bfrt_info, target)
        print(f'  Python read time        : {time.time()-t0:.2f}s')
    ...
    print()
    if already_cleared:
        print('  (registers already cleared by fast_io)')
    else:
        print('  Clearing BF + CMS registers for next epoch...')
...
def main():
    parser = argparse.ArgumentParser(
        description='FlowLiDAR Prototype 9 — Control Plane (Traditional BF + sub-sketch solver)')
    ...
    parser.add_argument('--fast-io', dest='fastio_bin', default=None,
                        help='Path to fast_io C++ binary (enables hybrid bulk read/clear)')
    parser.add_argument('--snapshot-dir', default='/tmp/flowlidar_snapshot',
                        help='Where fast_io drops register binary dumps (default: /tmp/flowlidar_snapshot)')
    args = parser.parse_args()
    epoch_seconds = args.epoch
    pipe_id       = args.pipe
    fastio_bin    = args.fastio_bin
    snapshot_dir  = args.snapshot_dir
    if fastio_bin and not os.access(fastio_bin, os.X_OK):
        print(f'[ERROR] fast_io binary not executable: {fastio_bin}'); sys.exit(1)

    print('=' * 72)
    print('  FlowLiDAR Prototype 9 — Control Plane (Traditional BF + sub-sketch solver)')
    ...
    # Extract register table IDs once so we can pass them to the C++ helper
    # without making it parse bfrt_info JSON itself.
    bf_table_ids  = [bfrt_info.table_get(f'pipe.SwitchIngress.{r}').info.id_get()
                     for r in BF_ROWS]
    cms_table_ids = [bfrt_info.table_get(f'pipe.SwitchIngress.{r}').info.id_get()
                     for r in CMS_ROWS]
    if fastio_bin:
        print(f'  fast_io BF  table ids : {bf_table_ids}')
        print(f'  fast_io CMS table ids : {cms_table_ids}')
    ...
    while True:
        try:
            ...
            process_epoch(epoch_num, flow_table, bfrt_info, target, total,
                          fastio_bin=fastio_bin, snapshot_dir=snapshot_dir,
                          pipe_id=pipe_id,
                          bf_table_ids=bf_table_ids,
                          cms_table_ids=cms_table_ids)
