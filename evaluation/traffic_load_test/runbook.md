...
Goal: at fixed lazy_bf1m / traditional_bf1m with CMS=64x1024, sweep the input traffic load and observe how coverage, accuracy, and solver-path distribution change as the BF and CMS get progressively stressed. The 64-bucket geometry plus `kSlowSolverCap = 5000` should let alg6 fire on heavy loads (8M+) — that's the new piece this experiment exposes.
...
| Variant | Type |
|---|---|
| `cpp_lazy_bf1m_cms64x1024` | Lazy BF |
| `cpp_traditional_bf1m_cms64x1024` | Standard BF |
...
| 4M  | `load_4000000_legacy.pcap`  | `load_4000000_truth.csv`  | |
| 8M  | `load_8000000_legacy.pcap`  | `load_8000000_truth.csv`  | alg6 expected to start firing on lazy |
| 16M | `load_16000000_legacy.pcap` | `load_16000000_truth.csv` | alg6 firing on both modes likely |
| 32M | `load_32000000_legacy.pcap` | `load_32000000_truth.csv` | heaviest; BF saturated |
...
# both CPs present and built with commit 8ff2d20 (cap=5000)?
ls -lh ~/dainius/cpp_{lazy,traditional}_bf1m_cms64x1024/*_cp

# both P4 programs installed?
ls ~/sde/bf-sde-9.11.0/install/share/p4/targets/tofino/lazy_bf1m_cms64x1024.conf \
   ~/sde/bf-sde-9.11.0/install/share/p4/targets/tofino/traditional_bf1m_cms64x1024.conf
```

If either .conf is missing, run the variant's `build.sh` on the switch (~10-15 min to compile + install the data-plane program).
...
All 4 register tables share the same IDs across both 64x1024 variants (confirmed by `print_ids.py` on both lazy and traditional). Use these directly:
```bash
export BF_IDS=2338372543,2333775667,2347111112
export CMS_IDS=2346354927,2339499036,2335880110
```

(If for some reason they differ on a future variant, run `python3 /home/onie/dainius/cpp_<variant>/print_ids.py` after switchd + setup_table to get the live values.)
...
mkdir -p ~/results_traffic_load_test/lazy_bf1m_cms64x1024
export BF_IDS=2338372543,2333775667,2347111112
export CMS_IDS=2346354927,2339499036,2335880110

~/dainius/cpp_lazy_bf1m_cms64x1024/lazy_bf1m_cms64x1024_cp \
  --epoch $EPOCH --pipe 1 --epochs 1 \
  --bf-ids $BF_IDS --cms-ids $CMS_IDS \
  --csv-out ~/results_traffic_load_test/lazy_bf1m_cms64x1024/est_load_${LOAD}.csv 2>&1 \
  | tee /tmp/lazy_bf1m_cms64x1024_load${LOAD}.log
```
...
```bash
grep -E "Resolved by Alg|Sub-sketch buckets|cms_[012] :" /tmp/lazy_bf1m_cms64x1024_load${LOAD}.log
```

Expected pattern (key thing to watch): `Sub-sketch buckets used: 64 / 64  (exact: X, Alg6 approx: Y, skipped: 0)`. At low loads X=64, Y=0. At 8M+ loads, Y should start non-zero. At 16M+, Y might dominate.

---

## Variant B — `traditional_bf1m_cms64x1024` (6 runs)

Identical ritual, just substitute the variant name.

**Terminal A:**
```bash
$SDE/run_switchd.sh -p traditional_bf1m_cms64x1024
```

**Terminal B — setup:**
```bash
bfshell
bfrt_python /home/onie/dainius/cpp_traditional_bf1m_cms64x1024/setup_table.py
# Ctrl+D
```
...
mkdir -p ~/results_traffic_load_test/traditional_bf1m_cms64x1024
export BF_IDS=2338372543,2333775667,2347111112
export CMS_IDS=2346354927,2339499036,2335880110

~/dainius/cpp_traditional_bf1m_cms64x1024/traditional_bf1m_cms64x1024_cp \
  --epoch $EPOCH --pipe 1 --epochs 1 \
  --bf-ids $BF_IDS --cms-ids $CMS_IDS \
  --csv-out ~/results_traffic_load_test/traditional_bf1m_cms64x1024/est_load_${LOAD}.csv 2>&1 \
  | tee /tmp/traditional_bf1m_cms64x1024_load${LOAD}.log
```

**Hotpot — replay** (same as Variant A, with the LOAD pcap).

**Sanity check** same grep.
...
for v in lazy_bf1m_cms64x1024 traditional_bf1m_cms64x1024; do
...
- 5 figures: coverage, ARE, AAE, exact rate, **alg6 share** (new — proves alg6 actually firing at heavy loads)
...
Per run: ~3 min switchd boot + 30 s setup + epoch (20-75 s) + bulk read + ~10 s scp = 5-7 min depending on load. 12 runs × 6 min average = **~70-80 min total switch time**.

If hotpot replay is slow at 32M, the 32M runs can take 90+ s each.
