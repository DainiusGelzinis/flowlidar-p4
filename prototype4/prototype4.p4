/* =============================================================================
 * prototype4.p4 — FlowLiDAR Prototype 4
 *
 * Goal: Replace the Standard BF (Algorithm 1) with the Lazy Updates BF
 * (Algorithm 2) from the FlowLiDAR paper.
 *
 * Key difference from prototype3:
 *   - BF arrays are checked sequentially; only the FIRST zero bit is set
 *   - bf_0 always executes; bf_1 only if b0==1; bf_2 only if b0==1 AND b1==1
 *   - CMS is only incremented when all 3 BF bits were already 1
 *   - Flows with ≤ k=3 packets never touch the CMS (counted via digest)
 *
 * Algorithm 2 (Lazy Updates BF):
 *   For each packet with FlowID x:
 *     for i = 1 to k:
 *       if bf_i[h_i(x)] == 0:
 *           Set bf_i[h_i(x)] = 1
 *           Send FlowID to control plane
 *           break
 *       else if i == k:
 *           Send x to CMS (packet counting block)
 *
...
control SwitchIngress(
    ...
    //
    // Stage 3: tbl_bf0 — always check-and-set bf_0
    // Stage 4: tbl_bf1 — only if b0 == 1 (table match)
    // Stage 5: tbl_bf2 — only if b0 == 1 AND b1 == 1 (table match)
    //
    // setup_table.py adds entries:
    //   tbl_bf1: b0=1 → run_bf1
    //   tbl_bf2: (b0=1, b1=1) → run_bf2
    // Default actions (skip) set the metadata bit to 0.
    // =========================================================================
    ...
    //
    // CMS is only incremented when all 3 BF bits were already 1 (known flow).
    // Each increment is wrapped in a table keyed on (b0, b1, b2).
    // setup_table.py adds entry (1,1,1) → do_cms_inc_i for each table.
    // =========================================================================
    ...
    // CMS hash functions — same polynomials as prototype3.
    ...
    apply {
        ...
        if (hdr.ipv4.isValid()) {
            ...
            // bf_0 always executes.
            // bf_1 only executes if b0 == 1 (table match).
            // bf_2 only executes if b0 == 1 AND b1 == 1 (table match).
            tbl_bf0.apply();
            tbl_bf1.apply();
            tbl_bf2.apply();

            // Step 5: Digest — send if any BF bit was 0 (new/partial flow).
            // When a BF array is skipped, its metadata bit is 0 (skip action),
            // so this correctly triggers the digest.
            if (ig_md.b0 == 0) { ig_dprsr_md.digest_type = 1; }
            if (ig_md.b1 == 0) { ig_dprsr_md.digest_type = 1; }
            if (ig_md.b2 == 0) { ig_dprsr_md.digest_type = 1; }

            // Step 6: Compute CMS hash indices (stage 6).
            tbl_cms_hash.apply();

            // Step 7: Conditional CMS increment (stages 7, 8, 9).
            // Only fires when b0==b1==b2==1 (all BF bits already set).
            tbl_cms_0.apply();
            tbl_cms_1.apply();
            tbl_cms_2.apply();
