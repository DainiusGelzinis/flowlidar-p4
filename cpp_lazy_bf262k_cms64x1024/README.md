# cpp_lazy_bf262k_cms64x1024 — pure C++ control plane, doubled BF

Pure-C++ FlowLiDAR control plane on real Tofino 1 (p4switch2). Variant
of `cpp_lazy_bf131k_cms64x1024` with the Bloom Filter doubled in size:

- **Data plane**: lazy BF (3 × **262144** × 1 bit, bit<18>) + sub-sketch CMS
  (3 × 65536 × 16-bit, 64 buckets × 1024 cols). CMS unchanged from the
  131k variant — only the BF grew.
- **Control plane**: a single C++ binary (`cp_cpp/lazy_bf262k_cp`) that owns the
...
├── lazy_bf262k.p4            P4 (lazy BF + sub-sketch CMS, prototype5/6 size)
...
├── README.md             this file
└── cp_cpp/
    ├── Makefile          builds lazy_bf262k_cp using bfrt-grpc + auto-generated stubs
    ├── main.cpp          entrypoint + per-epoch loop
    ├── bfrt_client.{hpp,cpp}  gRPC wrapper: subscribe, BIND, register I/O, digests
    ├── crc.hpp           CRC-32 family hashes matching lazy_bf262k.p4
...
./build.sh                  # compiles lazy_bf262k.p4 with bf-p4c
cd cp_cpp && make           # builds lazy_bf262k_cp
```
...
~/dainius/hardware_version_cpp_lazy_BF/cp_cpp/lazy_bf262k_cp \
    --epoch 30 --pipe 1
```
