#!/usr/bin/env python3
"""
FlowLiDAR hardware_version — Test Script
Run this on hotpot, not on the switch.

Same traffic pattern as prototype5. Ground truth is unchanged:
  Flow A: 12 packets, Flow B: 6, Flow C: 3, Flow D/E/F: 2 each.

Expected epoch report (all exact via postprocessing):
  Flow A: digest=3  solver=9   total=12  (equation solver)
  Flow B: digest=3  solver=3   total=6   (equation solver)
  Flow C: digest=3  alg5=0     total=3   (CMS preprocessing)
  Flow D: digest=2  alg4       total=2   (BF preprocessing)
  Flow E: digest=2  alg4       total=2   (BF preprocessing)
  Flow F: digest=2  alg4       total=2   (BF preprocessing)
...
SEND_IFACE = 'enp172s0f0np0'   # 40G interface on hotpot connected to p4switch2 port 1/0
DELAY      = 0.05
DIGEST_GAP = 1.5
...
print('=' * 64)
print('  FlowLiDAR hardware_version — Equation Solver Test')
print(f'  Sending on interface: {SEND_IFACE}')
print('  Make sure control_plane.py is running on p4switch2.')
print('=' * 64)
print()
