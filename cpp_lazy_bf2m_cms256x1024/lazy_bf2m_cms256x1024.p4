 ...
 *   BF  : 3 × 2097152 × 1 bit   (bit<20> index, k=3, lazy chain)
 ...
 *
 * Index widths:
 *   - BF register index : bit<20>  (2097152 cells)
...
struct metadata_t {
    ...
    // BF hash indices — 20-bit each (2097152 cells per row).
    bit<21> idx0;
    bit<21> idx1;
    bit<21> idx2;
...
control SwitchIngress(
    ...
    // =========================================================================
    // BLOOM FILTER — Lazy Updates, 3 × 2097152 cells (1 Mbit each)
    // =========================================================================

    Register<bit<1>, bit<21>>(2097152) bf_0;
    Register<bit<1>, bit<21>>(2097152) bf_1;
    Register<bit<1>, bit<21>>(2097152) bf_2;

    RegisterAction<bit<1>, bit<21>, bit<1>>(bf_0) bf_check_set_0 = {
        void apply(inout bit<1> val, out bit<1> rv) {
            rv  = val;
            val = 1;
        }
    };
    RegisterAction<bit<1>, bit<21>, bit<1>>(bf_1) bf_check_set_1 = {
        void apply(inout bit<1> val, out bit<1> rv) {
            rv  = val;
            val = 1;
        }
    };
    RegisterAction<bit<1>, bit<21>, bit<1>>(bf_2) bf_check_set_2 = {
        void apply(inout bit<1> val, out bit<1> rv) {
            rv  = val;
            val = 1;
        }
    ...
    Hash<bit<21>>(HashAlgorithm_t.CUSTOM, poly0) hash0;

    CRCPolynomial<bit<32>>(32w0x04C11DB7,
                           false, false, false,
                           32w0xFFFFFFFF, 32w0xFFFFFFFF) poly1;
    Hash<bit<21>>(HashAlgorithm_t.CUSTOM, poly1) hash1;

    CRCPolynomial<bit<32>>(32w0x1EDC6F41,
                           true, false, false,
                           32w0xFFFFFFFF, 32w0xFFFFFFFF) poly2;
    Hash<bit<21>>(HashAlgorithm_t.CUSTOM, poly2) hash2;
    ...
    // CMS column hashes — bit<10> output (1024 cols per sub-sketch).
    ...
    // Stage 8: combine sketchlet_offset with each col hash (18-bit add).
