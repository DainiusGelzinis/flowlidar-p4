#!/usr/bin/env python3
"""Per-class ARE / AAE breakdown at 8M load, lazy vs standard.

Reads /tmp/per_class_8m.csv (two rows: lazy and standard) and emits four
figures under plots_per_class_8m/:
  - are.png        ARE per class, log y, two lines
  - are_linear.png ARE per class, linear y, two lines
  - aae.png        AAE per class, log y, two lines
  - aae_linear.png AAE per class, linear y, two lines
"""
...
SRC  = "/tmp/per_class_8m.csv"
OUT  = os.path.join(HERE, "plots_per_class_8m")
...
with open(SRC) as f:
    for row in csv.DictReader(f):
        key = row["variant"].split(",", 1)[0]    # strip the meta tail
        rows[key] = row
