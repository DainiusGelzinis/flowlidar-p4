 ...
 *   BF  : 3 × 524288 × 1 bit   (bit<20> index, k=3, lazy chain)
 ...
 *
 * Index widths:
 *   - BF register index : bit<20>  (524288 cells)
...
struct metadata_t {
    ...
    // BF hash indices — 20-bit each (524288 cells per row).
    bit<19> idx0;
    bit<19> idx1;
    bit<19> idx2;
...
control SwitchIngress(
    ...
    // =========================================================================
    // BLOOM FILTER — Lazy Updates, 3 × 524288 cells (1 Mbit each)
    // =========================================================================

    Register<bit<1>, bit<19>>(524288) bf_0;
    Register<bit<1>, bit<19>>(524288) bf_1;
    Register<bit<1>, bit<19>>(524288) bf_2;

    RegisterAction<bit<1>, bit<19>, bit<1>>(bf_0) bf_check_set_0 = {
        void apply(inout bit<1> val, out bit<1> rv) {
            rv  = val;
            val = 1;
        }
    };
    RegisterAction<bit<1>, bit<19>, bit<1>>(bf_1) bf_check_set_1 = {
        void apply(inout bit<1> val, out bit<1> rv) {
            rv  = val;
            val = 1;
        }
    };
    RegisterAction<bit<1>, bit<19>, bit<1>>(bf_2) bf_check_set_2 = {
        void apply(inout bit<1> val, out bit<1> rv) {
            rv  = val;
            val = 1;
        }
    ...
    Hash<bit<19>>(HashAlgorithm_t.CUSTOM, poly0) hash0;
