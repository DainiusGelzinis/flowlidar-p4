# FlowLiDAR — Hardware Version 3 (Real Tofino 1, Prototype 7: doubled BF + Sub-Sketches)

Same workflow as `hardware_version2/`, but runs **prototype7**: BF doubled
to 3 × 262144 × 1 bit (was 3 × 131072) to halve per-cell load. CMS
sub-sketch partitioning (64 × 1024 cells per row) is unchanged.
...
Files live at `~/dainius/hardware_version3/` on the switch.

```bash
# Local VM → switch (whole directory)
scp hardware_version3/{prototype7.p4,build.sh,setup_table.py,control_plane.py,reset_epoch.py,test_packet.py} onie.two.hotpot:~/dainius/hardware_version3/
...
```bash
$SDE/run_switchd.sh -p prototype7
```
...
```
bfrt_python ~/dainius/hardware_version3/setup_table.py
```
...
```bash
python3 ~/dainius/hardware_version3/control_plane.py --epoch 30
```
...
| File | Purpose |
|------|---------|
| `prototype7.p4` | P4 program (BF lazy updates + master-hash sub-sketch CMS) |
...
## Differences from `hardware_version2/` (prototype6)

| Aspect | hardware_version2 | hardware_version3 |
|--------|-------------------|-------------------|
| P4 program | `prototype6` | `prototype7` |
| BF | 3 × 131072 × 1 bit | **3 × 262144 × 1 bit** (doubled) |
| BF index width | bit<17> | bit<18> |
| BF SRAM blocks per stage | 1 | 2 |
| CMS | 3 × 65536 × 16-bit (64 × 1024) | identical |
| Stages used | 0–11 | 0–11 (compiler may shift placement) |
| Expected coverage @ 1 Gbps / 15 s | ~24% | TBD (target: much higher) |
